...
- [Development guidelines](#development-guidelines)
    - [General DCT Core](#general-idchaincoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
DCT Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play DCTs (tDCT)" on the test network, if you
...
DCT Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/idchaincoin-project/idchaincoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_idchaincoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_idchaincoin --log_level=test_suite
$ valgrind -v --leak-check=full src/idchaincoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_idchaincoin.coverage/index.html`.
```
...
- BitcoinMiner : Generates DCTs (if wallet is enabled).
...
However, in open source software such as DCT Core, where everyone uses
...
pay attention to for reviewers of DCT Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: DCT Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
- Use `#!/usr/bin/env bash` instead of obsolete `#!/bin/bash`.
  ...
  - [*Rationale*](https://github.com/dylanaraps/pure-bash-heirsle#shebang):
...
be sent upstream but bugfixes may also be prudent to PR against DCT Core so that they can be integrated
...
file descriptors it will cause problems with DCT's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:DCT-Project/DCT.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `idchaincoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `idchaincoin-cli` and the GUI debug console use this table to determine how to
