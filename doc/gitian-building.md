...
*Setup instructions for a gitian build of DCT Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the DCT
...
to the DCT GitHub release page.
...
- [Building DCT Core](#building-idchaincoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/idchaincoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/idchaincoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/idchaincoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/idchaincoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/idchaincoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/idchaincoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/idchaincoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@idchaincoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/idchaincoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/idchaincoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/idchaincoin-osx-*-build.assert
```
