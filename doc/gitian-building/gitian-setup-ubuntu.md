...
The build script we'll be using is contained in the DCT github repository ([contrib/gitian-build.py](https://github.com/idchaincoin-project/idchaincoin/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the DCT repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/DCT-Project/DCT/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, idchaincoin-detached-sigs, and idchaincoin GitHub repos.
