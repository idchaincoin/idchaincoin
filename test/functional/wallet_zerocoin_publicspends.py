...
class ZerocoinSpendTest(IDChainCoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zheirs_bal, heirs_bal):
            zheirs_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zheirs_bal)
            heirs_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], heirs_bal)
            return zheirs_bal, heirs_bal
        ...
        zheirs_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zheirs_balance, 6666)
        ...
        zheirs_balance, balance = check_balances(denom_2, zheirs_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zheirs_balance, balance = check_balances(denom_3, zheirs_balance, balance)
        ...
        _, _ = check_balances(denom_1, zheirs_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
