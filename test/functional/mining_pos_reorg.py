...
from test_framework.test_framework import IDChainCoinTestFramework
...
class ReorgStakeTest(IDChainCoinTestFramework):
    ...
    def check_money_supply(self, expected_heirs, expected_zheirs):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected DCT and zDCT supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_heirs))
            for denom in node['zDCTsupply']:
                assert_equal(node['zDCTsupply'][denom], DecimalAmt(expected_zheirs[denom]))
    ...
    def run_test(self):
        ...
        # Check DCT and zDCT supply at the beginning
        # ------------------------------------------
        # zDCT supply: 2 coins for each denomination
        expected_zheirs_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # DCT supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zheirs_supply)
        ...
        # Verify that DCT and zDCT supplies were properly updated after the spends and reorgs
        self.log.info("Check DCT and zDCT supply...")
        ...
        expected_zheirs_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zheirs_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zheirs_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zheirs_supply)
