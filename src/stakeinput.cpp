...
bool CHeirsStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CHeirsStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CHeirsStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CHeirsStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CHeirsStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CHeirsStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CHeirsStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CHeirsStake::GetUniqueness() const
{
    //The unique identifier for a DCT stake is the outpoint
...
CBlockIndex* CHeirsStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CHeirsStake::ContextCheck(int nHeight, uint32_t nTime)
{
