...
#include "qt/idchaincoin/dashboardwidget.h"
#include "qt/idchaincoin/forms/ui_dashboardwidget.h"
#include "qt/idchaincoin/sendconfirmdialog.h"
#include "qt/idchaincoin/txrow.h"
...
DashboardWidget::DashboardWidget(DCTGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of DCT earned via Staking & Masternodes"));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareHeirs, "square-chart-heirs");
    setCssProperty(ui->labelHeirs, "text-chart-heirs");
    ...
    setCssProperty(ui->labelAmountHeirs, "text-stake-heirs-disable");
	ui->labelAmountMNRewards->setText("0 DCT");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &DCTGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &DCTGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &DCTGUI::openFAQ);
...
void DashboardWidget::changeChartColors()
{
    ...
    QColor gridY;
    if (isLightTheme()) {
        gridLineColorX = QColor(48,85,178,0.3);
        ...
        gridLineColorX = QColor(18,25,104,0.7);
        linePenColorY =  gridLineColorX;
        backgroundColor = linePenColorY;
    }
...
const QMap<int, QMap<QString, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isHeirs = modelIndex.data(TransactionTableModel::TypeRole).toInt() == TransactionRecord::StakeMint;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair DCT, zDCT
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            heirs = (pair["heirs"] != 0) ? pair["heirs"] / 100000000 : 0;
            mnrewards = (pair["mn"] != 0) ? pair["mn"] / 100000000 : 0;
            chartData->totalHeirs += pair["heirs"];
            chartData->totalMNRewards += pair["mn"];
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesHeirs.append(heirs);    
        chartData->valuesMNRewards.append(mnrewards);    

        int max = std::max(heirs, mnrewards);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(99,103,204));
	set1->setColor(QColor(38,32,117));
    ...
    set0->append(chartData->valuesHeirs);
    set1->append(chartData->valuesMNRewards);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalHeirs > 0 || chartData->totalMNRewards > 0) {
        setCssProperty(ui->labelAmountHeirs, "text-stake-heirs");
	...
	forceUpdateStyle({ui->labelAmountHeirs, ui->labelAmountMNRewards});
    ui->labelAmountHeirs->setText(GUIUtil::formatBalance(chartData->totalHeirs, nDisplayUnit));
