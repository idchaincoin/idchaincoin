...
#include "qt/idchaincoin/forms/ui_snackbar.h"
#include "qt/idchaincoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(DCTGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &DCTGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
