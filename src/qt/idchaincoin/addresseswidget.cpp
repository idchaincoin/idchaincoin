...
#include "qt/idchaincoin/forms/ui_addresseswidget.h"
#include "qt/idchaincoin/addresslabelrow.h"
#include "qt/idchaincoin/addnewaddressdialog.h"
#include "qt/idchaincoin/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(DCTGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination heirsAdd = DecodeDestination(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(
            heirsAdd, label.toUtf8().constData(),AddressBook::AddressBookPurpose::SEND
        )) {
