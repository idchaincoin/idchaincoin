...
#include "qt/idchaincoin/topbar.h"
#include "askpassphrasedialog.h"
#include "qt/idchaincoin/forms/ui_topbar.h"
#include "qt/idchaincoin/loadingdialog.h"
#include "qt/idchaincoin/lockunlock.h"
#include "qt/idchaincoin/qtutils.h"
#include "qt/idchaincoin/receivedialog.h"
...
TopBar::TopBar(DCTGUI* _mainWindow, QWidget* parent) : PWidget(_mainWindow, parent),
                                                        ui(new Ui::TopBar)
{
    ui->setupUi(this);
    ...
    setCssProperty({ui->labelAmountTopHeirs}, "amount-small-topbar");
    setCssProperty({ui->labelAmountHeirs}, "amount-topbar");
    setCssProperty({ui->labelPendingHeirs, ui->labelImmatureHeirs, ui->labelAvailableHeirs,
                       ui->labelLockedHeirs, ui->labelMasternodeCount, ui->labelCollateralHeirs,
    ...
    ui->pushButtonConf->setButtonText("idchaincoin.conf");
...
void TopBar::onBtnConfClicked()
{
    ...
    if (!GUIUtil::openConfigfile())
        inform(tr("Unable to open idchaincoin.conf with default application"));
}
...
void TopBar::refreshStatus()
{
    ...
    // Collateral
    ui->labelCollateralHeirs->setText(GUIUtil::formatBalance(CMasternode::GetMasternodeNodeCollateral(chainActive.Tip()->nHeight), nDisplayUnit));
}
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // DCT
    // Top
    ui->labelAmountTopHeirs->setText(GUIUtil::formatBalance(nAvailableBalance, nDisplayUnit));
    // Expanded
    ui->labelAmountHeirs->setText(GUIUtil::formatBalance(newBalance.balance + newBalance.immature_balance, nDisplayUnit));
    ui->labelAvailableHeirs->setText(GUIUtil::formatBalance(nAvailableBalance, nDisplayUnit));
    ui->labelPendingHeirs->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance, nDisplayUnit));
    ui->labelImmatureHeirs->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
    ui->labelLockedHeirs->setText(GUIUtil::formatBalance(nLockedBalance, nDisplayUnit));
