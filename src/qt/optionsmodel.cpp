...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::DCT);
    ...
    if (settings.contains("nAnonymizeIDChainCoinAmount") || reset)
        SoftSetArg("-anonymizeidchaincoinamount", settings.value("nAnonymizeIDChainCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in idchaincoin.cpp during shotdown to also remove the window positions
