...
#include "config/idchaincoin-config.h"
#endif
...
#include "qt/idchaincoin/splash.h"
#include "qt/idchaincoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("idchaincoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in idchaincoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in idchaincoin.qrc)
...
/** Class encapsulating DCT Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (DCTGUI)
...
{
    window = new DCTGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &DCTGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &DCTGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(DCTGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(DCTGUI::DEFAULT_WALLET);
        ...
        // DCT: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &DCTGUI::handlePaymentRequest);
        connect(window, &DCTGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. DCT can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(idchaincoin_locale);
    Q_INIT_RESOURCE(idchaincoin);
    ...
    /// 6. Determine availability of data directory and parse idchaincoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("DCT"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("DCT"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // idchaincoin: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("DCT didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
