...
#if defined(NDEBUG)
#error "DCT cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uDCT) are considered zero fee (for relaying, mining and transaction creation)
...
{
    util::ThreadRename("idchaincoin-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> heirsInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
           {
                heirsInputs.push_back(stakeIn);
            }
        }
        const bool hasDCTInputs = !heirsInputs.empty();
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasDCTInputs) {
                            for (const CTxIn& stakeIn : heirsInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // DCT: We use certain sporks during IBD, so check to see if they are
